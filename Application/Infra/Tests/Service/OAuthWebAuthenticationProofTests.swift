//
//  OAuthWebAuthenticationProofTests.swift
//  InfraTests
//
...
struct OAuthWebAuthenticationProofTests {
    ...
    func app_challenge는_app_verifier의_SHA_256_base64url_값이다() {
        ...
        #expect(proof.appChallenge == "E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM")
    }
    ...
    func 생성된_app_verifier는_64자리_소문자_16진수다() {
        ...
        #expect(proof.appVerifier.count == 64)
        #expect(proof.appVerifier.unicodeScalars.allSatisfy(allowedCharacters.contains))
    }

    @Test("DevLog OAuth callback에서 ticket을 추출한다")
    func DevLog_OAuth_callback에서_ticket을_추출한다() throws {
        let proof = OAuthWebAuthenticationProof(appVerifier: "verifier")
        let callbackURL = try #require(
            URL(string: "DevLog://oauth-callback?ticket=one-time-ticket")
        )

        #expect(try proof.ticket(from: callbackURL) == "one-time-ticket")
    }
    ...
    func 다른_callback_host는_거부한다() throws {
        ...
        #expect(throws: SocialLoginError.self) {
            try proof.ticket(from: callbackURL)
        }
    }

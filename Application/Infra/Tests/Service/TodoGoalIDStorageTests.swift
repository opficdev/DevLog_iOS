...
struct TodoGoalIDStorageTests {
    @Test("목표 연결 해제 요청은 goalId 삭제 값을 만든다")
    func 목표_연결_해제_요청은_goalId_삭제_값을_만든다() throws {
        let data = try TodoServiceImpl.makeDocumentData(from: makeRequest(goalId: nil))
    ...
    func 목표_연결_요청은_goalId_값을_저장한다() throws {
        let data = try TodoServiceImpl.makeDocumentData(from: makeRequest(goalId: "goal-1"))
    ...
    func goalId_누락과_null은_연결되지_않은_Todo로_읽는다() throws {
        let missing = try #require(
            TodoServiceImpl.makeResponse(documentID: "todo-1", data: makeDocumentData(goalId: nil))
        )
        let null = try #require(
            TodoServiceImpl.makeResponse(documentID: "todo-2", data: makeDocumentData(goalId: NSNull()))
        )

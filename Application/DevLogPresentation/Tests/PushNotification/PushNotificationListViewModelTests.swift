...
@MainActor
struct PushNotificationListViewModelTests {
    ...
    func fetchNotifications는_첫_페이지를_조회하고_목록과_hasMore_상태를_갱신한다() async throws {
        ...
        let adapter = PushNotificationListViewModelTestAdapter(fetchUseCase: fetchSpy)
    ...
    func 필터_액션은_query와_적용_필터_수를_갱신한다() async throws {
        ...
        let adapter = PushNotificationListViewModelTestAdapter(updateQueryUseCase: updateSpy)
    ...
    func selectNotification은_선택_상태를_바꾸고_읽지_않은_알림을_읽음_처리한다() async throws {
        ...
        let adapter = PushNotificationListViewModelTestAdapter(
